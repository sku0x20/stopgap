...
class ClientsManager : LauncherSessionListener {
    ...
    override fun launcherSessionOpened(session: LauncherSession) {
        val port = session.store.get(Namespace.create(DockerImageRunner::class.java), Int::class.java) as Int
        clients = ManagedClients()
        clients.setup("localhost", port)
        session.store.put(namespace, ManagedClients::class.java, clients)
    }
